...
class _GuessChartByInfoPageState extends State<GuessChartByInfoPage> {
  ...
  Future<void> _loadStats() async {
    // 直接获取最新统计数据，不需要重置数据
    final stats = await _cacheUtil.getStats('1');
    setState(() {
  ...
  Future<void> _recordGameResult(bool isWon) async {
    ...
    // 记录结果
    if (isWon) {
      await _cacheUtil.recordSuccess('1');
    } else {
      await _cacheUtil.recordFailure('1');
    }
    
    // 结算游戏
    await _cacheUtil.settleGame('1', gameTime);
                                      ...
                                      ? Column(
                                              ...
                                              if (_timeLimit > 0)
                                                Text(
                                                  ...
                                                  style: TextStyle(
                                                      fontSize:
                                                          screenWidth * 0.04,
                                                      color: _remainingTime / _timeLimit <= 0.3
                                                          ? Colors.red // 小于等于30%时变为红色
                                                          : Colors.black
                                                  ),
                                                ),
                                              // 统计数据
                                        ...
                                        Container(
                                          ...
                                          child: Row(
                                            ...
                                            children: [
                                              // 按钮容器，使用Row布局并设置mainAxisSize为min
                                              Row(
