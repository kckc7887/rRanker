import { enrichRecordsWithCatalog, isUtageSongId } from './catalog';
import { buildBest50 } from './rating';
import type {
  CatalogSnapshot,
  Player,
  ScoreRecord,
...
import type { CatalogRepository, SnapshotRepository } from './repositories';
...
export function buildScoreSnapshot(
  ...
  const derivesRatingFromBest50 =
    player.source.kind === 'local' || player.source.kind === 'generated';
...
function withoutUtageRecords(snapshot: ScoreSnapshot): ScoreSnapshot {
  ...
  const b35 = snapshot.best50.b35.filter(
    (record) => !isUtageSongId(record.songId),
  );
  ...
  return {
    ...
    best50: {
      ...
      unmatchedRecordCount: Math.max(
        0,
        snapshot.best50.unmatchedRecordCount - removed,
      ),
    },
  };
}
    ...
    try {
      ...
      if (cached) {
        ...
        const needsLogin =
          ...
          (error.code === 'authentication' || error.code === 'permission');
        return {
          ...sanitized,
          source: {
            ...sanitized.source,
            kind: 'cache',
            label: `${
              needsLogin ? '登录已失效，请重新登录；' : ''
            }最近有效成绩快照（原：${sanitized.source.label}）`,
            isStale: true,
          },
        };
      }
