...
export function chartVersionKey(
...
export function enrichRecordsWithCatalog(
  ...
  return records.map((record) => {
    ...
    const versionId =
      catalog.chartVersionIndex[chartVersionKey(record.songId, record.type, record.levelIndex)];
    return {
      ...record,
      difficultyConstant: chart?.difficultyConstant ?? record.difficultyConstant,
      notes: chart?.notes ?? record.notes,
      version:
        versionId === undefined ? 'unknown' : (versionTitles.get(versionId) ?? String(versionId)),
    };
  });
}
