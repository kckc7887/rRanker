...
export function calculateChartRating(
  ...
  const coefficient =
    RATING_COEFFICIENTS.find(([threshold]) => achievements < threshold)?.[1] ?? 22.4;
...
export function ratingTable(
  difficultyConstant: number,
): { achievement: number; rating: number }[] {
  return RATING_COEFFICIENTS.filter(([achievement]) => Number.isFinite(achievement)).map(
    ([achievement]) => ({
      achievement,
      rating: calculateChartRating(difficultyConstant, achievement),
    }),
  );
}

export function ratingTableDescending(
  difficultyConstant: number,
): { achievement: number; rating: number }[] {
  return ratingTable(difficultyConstant).reverse();
}

export function minimumAchievementForRating(
  difficultyConstant: number,
  targetRating: number,
): number | null {
  if (
    !Number.isFinite(difficultyConstant) ||
...
export function buildBest50(
  ...
  const b35 = rankScoreRecords(
    classified
      .filter(
        ({ version }) => version !== undefined && version !== catalog.currentVersion.id,
      )
      .map(({ record }) => record),
  ).slice(0, 35);
  const b15 = rankScoreRecords(
    classified
