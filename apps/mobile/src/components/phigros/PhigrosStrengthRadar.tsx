...
export function PhigrosStrengthRadar({
  ...
  const accessibilityLabel = `五维实力雷达，刻度 ${min.toFixed(1)} 到 ${max.toFixed(4)}。${tags
    .map((tag) => `${tag.name} ${tag.averageRks == null ? '无数据' : `修正后 ${tag.averageRks.toFixed(4)}，原始平均 ${tag.rawAverageRks!.toFixed(4)}，系数 ${tag.coefficient.toFixed(4)}，${tag.sampleCount}张入池谱面，${tag.eligibleChartCount}张候选谱面`}`)
    .join('；')}`;
