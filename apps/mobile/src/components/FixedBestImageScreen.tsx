...
export function FixedBestImageScreen({
  ...
  const [androidSource, setAndroidSource] = useState<BestImageWebViewSource | null>(null);
  ...
  useEffect(() => {
    ...
    setAndroidSource(prepared.sources[0] ?? null);
    return prepared.dispose;
  }, [html, pageId, width]);
  const source = Platform.OS === 'android' ? androidSource : inlineSource;
  const previewState = previewStates[pageId] ?? { phase: 'loading' as const, version: null };
  useBestImageWebViewTimeout(!!source, pageId, previewState.phase, setPreviewStates);
  ...
  const exportImage = async () => {
    if (!source || disabled || exporting) return;
  ...
  return <>
    <ScrollView style={[styles.page, { backgroundColor: theme.background }]} contentContainerStyle={styles.content}>
      ...
      <View accessibilityLabel="固定成绩图片预览" style={[styles.preview, { width: previewWidth, height: previewHeight, backgroundColor: theme.surface, borderColor: theme.border }]}>
        {source ? <WebView allowFileAccess={Platform.OS === 'android'} allowFileAccessFromFileURLs bounces={false}
          ...
          originWhitelist={['*']} scrollEnabled={false} source={source} style={styles.webview} testID="fixed-best-image-preview" />
          : <ActivityIndicator color={theme.accent} />}
      </View>
      <Text style={[styles.meta, { color: theme.textMuted }]}>{width} × {measuredHeight} px · {status}</Text>
      <Pressable accessibilityLabel="导出成绩图片" accessibilityRole="button" disabled={!source || disabled || exporting}
        onPress={() => void exportImage()} style={[styles.exportButton, { backgroundColor: theme.accent }, (!source || disabled || exporting) && styles.disabled]}>
    ...
    <Modal animationType="none" onRequestClose={() => undefined} transparent={false} visible={exporting}>
      {source ? <View style={styles.exportRoot}><View ref={exportRef} collapsable={false}
        ...
        <WebView allowFileAccess={Platform.OS === 'android'} allowFileAccessFromFileURLs androidLayerType="software"
          ...
          originWhitelist={['*']} scrollEnabled={false} source={source} style={styles.webview} />
...
const styles = StyleSheet.create({
  ...
  webview: { flex: 1, backgroundColor: 'transparent' }, meta: { marginTop: 8, fontSize: 11, textAlign: 'center' },
