...
export function chunithmJacketId(song: ChunithmSong): number {
  const worldsEnd = song.difficulties.find(
    (difficulty) => difficulty.difficulty === 5 && difficulty.originId !== undefined,
  );
  return worldsEnd?.originId ?? song.id;
}
