...
export function selectPreviewResources(input: {
  ...
  const visuals = parseBeatmapVisuals(input.osuText, sources.map(source => source.text), {
    osuPath: input.osuPath, osbPaths: sources.map(source => source.path),
  });
  ...
  return { osbPaths, imagePaths, videoPath: visuals.video ? lookup(visuals.video.file) ?? null : null, audioPaths: [...audio] };
}
