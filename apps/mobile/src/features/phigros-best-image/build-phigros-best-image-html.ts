...
export function ratingName(score: number, fc: unknown): string {
...
<div class="b19" data-layout-content>${scoreCards(input)}</div><div class="createdbox" data-layout-content><div class="phi-plugin"><p>Designed by Phi-Plugin. Data from TapTap.</p><p>Generated by rRanker</p></div></div></main>
<script>(()=>{
const W=${input.width},S=${scale},FALLBACK=${fallbackHeight},canvas=document.getElementById('canvas'),post=m=>window.ReactNativeWebView&&window.ReactNativeWebView.postMessage(JSON.stringify(m));
let last=FALLBACK;
...
const measureHeight=()=>{canvas.style.height='auto';const children=[...canvas.children].filter(node=>node.hasAttribute('data-layout-content')),bottom=children.reduce((maximum,node)=>{const style=getComputedStyle(node),marginBottom=parseFloat(style.marginBottom)||0;return Math.max(maximum,node.offsetTop+node.offsetHeight+marginBottom)},0);canvas.style.height=Math.max(1,Math.ceil(bottom))+'px';return Math.max(1,Math.ceil(bottom*S))};
const fit=()=>{last=measureHeight();const vw=innerWidth||document.documentElement.clientWidth||W,vh=innerHeight||document.documentElement.clientHeight||FALLBACK,exportView=Math.abs(vw-W)<2&&Math.abs(vh-last)<3,viewScale=exportView?1:Math.min(vw/W,vh/last),renderScale=S*viewScale;canvas.style.left=exportView?'0px':Math.max(0,(vw-W*viewScale)/2)+'px';canvas.style.top=exportView?'0px':Math.max(0,(vh-last*viewScale)/2)+'px';canvas.style.transform='scale('+renderScale+')';post({type:'best-image-height',width:W,height:last})};
