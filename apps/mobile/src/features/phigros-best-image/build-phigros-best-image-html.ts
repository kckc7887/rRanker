...
export function buildPhigrosBestImageHtml(input: PhigrosBestImageHtmlInput): string {
  ...
  return `<!doctype html><html lang="zh-cn"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>phi-plugin</title><style>
${templateCss}
...
<script>(()=>{const W=${input.width},FALLBACK=${fallbackHeight},canvas=document.getElementById('canvas'),post=m=>window.ReactNativeWebView&&window.ReactNativeWebView.postMessage(JSON.stringify(m));let last=FALLBACK;function adjustFontSize(){document.getElementsByName('pvis').forEach(node=>{const parent=node.parentElement;if(!parent)return;node.style.fontSize='';let size=parseFloat(getComputedStyle(node).fontSize)||16;if(node.scrollWidth<=parent.offsetWidth&&node.scrollHeight<=parent.offsetHeight)return;let low=0,high=Math.floor(size);while(low<high){const mid=Math.floor((low+high+1)/2);node.style.fontSize=mid+'px';if(node.scrollWidth>parent.offsetWidth||node.scrollHeight>parent.offsetHeight)high=mid-1;else low=mid}node.style.fontSize=low+'px'})}post({type:'best-image-runtime',width:W,userAgent:navigator.userAgent||''});const fit=()=>{canvas.style.transform='none';last=Math.max(1,Math.ceil(canvas.getBoundingClientRect().height));const vw=innerWidth||W,vh=innerHeight||FALLBACK,exportView=Math.abs(vw-W)<2&&vh+2>=last,viewScale=exportView?1:Math.min(vw/W,vh/last);canvas.style.left=exportView?'0px':Math.max(0,(vw-W*viewScale)/2)+'px';canvas.style.top=exportView?'0px':Math.max(0,(vh-last*viewScale)/2)+'px';canvas.style.transform='scale('+viewScale+')';post({type:'best-image-height',width:W,height:last})};addEventListener('resize',()=>{adjustFontSize();fit()});const images=[...document.images].map(i=>i.complete?Promise.resolve():new Promise(r=>{i.onload=i.onerror=r}));Promise.race([Promise.all([document.fonts?document.fonts.ready:Promise.resolve(),...images]),new Promise(r=>setTimeout(r,12000))]).then(()=>requestAnimationFrame(()=>requestAnimationFrame(()=>{adjustFontSize();fit();const m={type:'best-image-ready',width:W,height:last};post(m);setTimeout(()=>post(m),250)})));fit()})();</script></body></html>`;
}
