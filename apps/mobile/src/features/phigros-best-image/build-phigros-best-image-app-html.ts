...
function scoreCard(
  ...
  return renderApplicationBestImageCard({
    key: `${record.songId}:${rank}`,
    accessibilityLabel: `第 ${rank} 名 ${title}`,
    identifier: rank,
    title,
    coverUri: illustration ?? null,
    palette: {
      background: cardColor,
      border: 'rgba(255,255,255,.35)',
      text: '#FFFFFF',
      mutedText: 'rgba(255,255,255,.88)',
      metricText: 'rgba(255,255,255,.78)',
      faintText: 'rgba(255,255,255,.6)',
      suggestMutedText: 'rgba(255,255,255,.55)',
    },
    primary: {
      text: String(score),
      badge: { key: 'rate', variant: 'rate', label: rateColors.label, background: rateColors.bg, text: rateColors.fg },
      trailing: `${record.achievements.toFixed(2)}%`,
    },
    relation: {
      text: `${record.difficultyConstant.toFixed(1)}->${formatPhigrosSongRks(record.rating)}`,
      trailing: push.label,
      trailingMuted: push.type === null,
    },
  });
}
