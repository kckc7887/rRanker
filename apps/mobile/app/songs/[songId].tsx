...
function Detail({ song, records, catalogSource, scoreSource, library }: { song: Song; records: ScoreRecord[]; catalogSource: Song['charts'][number] extends never ? never : import('@/domain/models').DataSource; scoreSource?: import('@/domain/models').DataSource; library: ReturnType<typeof useUserLibrary> }) {
  ...
  return <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
    ...
    <Pressable accessibilityRole="button" disabled={library.isUpdating} onPress={() => void library.setSongFavorite(song.id, !favorite)} style={[styles.action, favorite && styles.actionActive]}>
      <Text style={[styles.actionText, favorite && styles.actionTextActive]}>{favorite ? '已收藏歌曲' : '收藏歌曲'}</Text>
    </Pressable>
    <SourceStatus items={[
    ...
    {song.charts.map((chart) => {
      ...
      return <Card key={`${chart.type}:${chart.levelIndex}`}><Text style={styles.section}>{chart.type} · {chart.difficulty.toUpperCase()} · {chart.level} ({chart.difficultyConstant.toFixed(1)})</Text>
        ...
        <Pressable accessibilityRole="button" disabled={library.isUpdating} onPress={() => void library.setChartPractice(song.id, chart.type, chart.levelIndex, !practice)} style={[styles.action, practice && styles.actionActive]}>
          <Text style={[styles.actionText, practice && styles.actionTextActive]}>{practice ? '已加入练习清单' : '加入练习清单'}</Text>
        </Pressable>
...
const styles = StyleSheet.create({ page: { flex: 1, backgroundColor: '#F7F8FA' }, content: { padding: 16, gap: 12 }, header: { flexDirection: 'row', gap: 14, alignItems: 'center' }, headerText: { flex: 1, gap: 3 }, title: { fontSize: 21, fontWeight: '800', color: '#111827' }, meta: { color: '#6B7280', fontSize: 12 }, section: { fontWeight: '700', color: '#111827', marginBottom: 7 }, body: { color: '#374151', lineHeight: 20 }, best: { color: '#246BFD', fontWeight: '600', marginTop: 6 }, action: { marginTop: 8, borderWidth: 1, borderColor: '#246BFD', borderRadius: 9, padding: 9, alignItems: 'center' }, actionActive: { backgroundColor: '#246BFD' }, actionText: { color: '#246BFD', fontWeight: '600' }, actionTextActive: { color: '#FFF' } });
