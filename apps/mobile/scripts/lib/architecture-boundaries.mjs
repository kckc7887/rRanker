...
export function inspectArchitectureSources(entries, games) {
  const root = resolve('src');
  const gameComponents = new Set(games);
  const violations = [];
  for (const { path, source: text } of entries) {
    const file = resolve(root, path);
    const source = ts.createSourceFile(file, text, ts.ScriptTarget.Latest, true);
    const componentGame = path.split('/')[0] === 'components' && gameComponents.has(path.split('/')[1]) ? path.split('/')[1] : null;
    const screenName = path.startsWith('screens/') ? path.slice('screens/'.length).replaceAll('-', '').toLowerCase() : '';
    const ownGame = componentGame ?? [...gameComponents].find((game) => screenName.startsWith(game.replaceAll('-', '').toLowerCase())) ?? null;
    ...
    function inspect(node) {
      ...
      if (specifier && ts.isStringLiteralLike(specifier)) {
        ...
        if (path.startsWith('state/') && layer === 'components') fail(node, `状态层反向依赖组件 ${target}`);
        if ((path.startsWith('storage/') || path.startsWith('features/storage-management/')) && layer === 'hooks') fail(node, `存储执行核心反向依赖 Hook ${target}`);
