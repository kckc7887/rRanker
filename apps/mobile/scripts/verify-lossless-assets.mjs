...
const baselineFiles = new Set(execFileSync('git', ['ls-tree', '--full-tree', '-r', '--name-only', `${baselineSha}:apps/mobile/assets`], { encoding: 'utf8' })
  .trim().split('\n').filter(Boolean).map((path) => `assets/${path}`));
assert(baselineFiles.size > 0, 'Baseline asset tree contains no files');
...
for (const path of files) {
  ...
  if (!baselineFiles.has(path)) {
    // New assets have no before-image in this baseline; validate integrity and
    // decoding without claiming a lossless comparison or optimization saving.
    recompressPng(current);
    const { info } = await sharp(current).ensureAlpha().raw().toBuffer({ resolveWithObject: true });
    addedFiles.push({ path, bytes: current.length, width: info.width, height: info.height });
    continue;
  }
...
writeFileSync('build/optimization-lossless-assets.json', JSON.stringify({ baselineSha, files: report, addedFiles }, null, 2) + '\n');
...
console.log(`Validated ${addedFiles.length} added PNGs for CRC and decoding; no baseline comparison or savings claimed.`);
