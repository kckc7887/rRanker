...
describe('Phigros 成绩图', () => {
  it('兼容旧样式偏好并保存合法的 OVER FLOW 数量与风格', () => {
    ...
    })).toMatchObject({ version: 2, ratingStyle: 'app', overflowCount: 3 });
    expect(parsePhigrosBestImageStylePreferences(null)).toMatchObject({ version: 2, ratingStyle: 'game' });
  });
  ...
  it('转义长文本、缺失素材回退，并输出三档分辨率', () => {
    ...
    for (const width of [1080, 1440, 2160] as const) {
      ...
      expect(html).toContain('<p>Designed by Phi-Plugin. Data from TapTap.</p>');
      expect(html).toContain('<p>Generated by rRanker</p>');
