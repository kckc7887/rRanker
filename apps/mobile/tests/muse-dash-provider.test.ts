...
import playDetail from './fixtures/musedash/play-detail.sanitized.json';
...
describe('MuseDashProvider', () => {
  it('parses sanitized albums, player, ce, diffdiff, play detail and search snapshots without credentials', async () => {
    const fetcher = vi.fn()
      ...
      .mockResolvedValueOnce(response(playDetail))
    ...
    const parsedDetail = await provider.getPlayDetail('13-5', 2, 'mobile', '6ea4f986ffd211e8aa980242ac110011');
    expect(parsedDetail).toMatchObject({ play: { miss: 0, judge: 'ss', combo: 471, acc: 95.47999572753906 } });
    expect(parsedDetail.user?.nickname).toBe('SiMOOOOOON');
  ...
  it('preserves search, player and play detail id query semantics', async () => {
    const fetcher = vi.fn().mockImplementation(async (url: string) =>
      response(url.includes('/rank/') ? playDetail : url.includes('/player/') ? player : search));
    ...
    await new MuseDashProvider(fetcher as typeof fetch, 'https://musedash.test')
      .getPlayDetail('13-5', 2, 'mobile', '6ea4f986ffd211e8aa980242ac110011');
    expect(fetcher.mock.calls[2][0])
      .toBe('https://musedash.test/rank/13-5/2/mobile/6ea4f986ffd211e8aa980242ac110011');
  });
