...
import OsuChartPreviewScreen from '../app/songs/osu-chart-preview';

const mockInject = jest.fn();
const mockDispose = jest.fn();
const mockPrepare = jest.fn(async (..._args: unknown[]) => ({
  uri: 'file:///osu-session/index.html', allowingReadAccessToURL: 'file:///osu-session/', dispose: mockDispose,
}));
...
jest.mock('expo-sqlite/kv-store', () => ({
  __esModule: true,
  default: {
    getItem: async () => JSON.stringify({ maniaSkin: 'circle', maniaScrollSpeed: 12 }),
...
jest.mock('@/theme/app-theme', () => ({ useAppTheme: () => ({
  ...
  accent: '#ff66aa', surfaceMuted: '#333333', statusBar: 'light',
}) }));
jest.mock('@/features/osu-chart-preview/prepare-osu-chart-preview-webview', () => ({
  prepareOsuChartPreviewWebViewSource: (...args: unknown[]) => mockPrepare(...args),
}));
...
describe('osu 谱面确认页面', () => {
  ...
  it('通过公共壳恢复设置、准备当前难度并在 ready 后撤下进度', async () => {
    const screen = await render(<OsuChartPreviewScreen />);
    ...
    const player = screen.getByTestId('osu-chart-preview-webview');
    expect(player.props.allowFileAccess).toBe(true);
    expect(player.props.allowingReadAccessToURL).toBe('file:///osu-session/');
    ...
    await waitFor(() => expect(mockSaveSettings).toHaveBeenCalledWith(
      ...
      JSON.stringify({ maniaSkin: 'circle', maniaScrollSpeed: 12, backgroundBlur: 5 }),
    ));
