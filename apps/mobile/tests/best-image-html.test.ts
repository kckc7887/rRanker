...
describe('best image html', () => {
  ...
  it('injects the Noto Sans font, single-line squeezed titles and the footer only for the game style', () => {
    ...
    expect(game).toContain("document.querySelectorAll('.game-title, .game-dxscore, .game-ds-rating')");
    expect(game).toContain("dx.offsetLeft - node.offsetLeft - 4");
    expect(game).toContain("'translate(-50%,-50%) ' + squeeze");
    expect(game).toContain("'translateY(-50%) ' + squeeze");
    expect(game).toContain("const squeeze = 'scaleX(' + (node.clientWidth / node.scrollWidth).toFixed(4) + ')'");
    expect(game).toContain('max-width:69px');
