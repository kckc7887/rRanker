...
describe('best image html', () => {
  ...
  it('renders the full-width app profile with adaptive identity, stars and fixed glass settings', () => {
    ...
    expect(app).toContain('<div class="app-player-row" id="player-row">');
    ...
    expect(app).toContain('class="app-course-rank" id="course-rank" aria-label="段位认定 裏皆伝"');
    expect(app).toContain('<span>段位认定</span><strong>裏皆伝</strong>');
    expect(app).toContain('.app-course-rank{display:inline-flex;height:28px;');
  ...
  it('injects the Noto Sans font, single-line squeezed titles and the footer only for the game style', () => {
    ...
    expect(game).toContain("document.querySelectorAll('.game-card').forEach((card) => {");
    expect(game).toContain("(dxCenter - dsLeft - gap) / (dsW + dxW / 2)");
    expect(game).toContain("(2 * (cardRight - dxCenter)) / dxW");
    expect(game).toContain("'translate(-50%,-50%) ' + squeeze");
    expect(game).toContain("'translateY(-50%) ' + squeeze");
    expect(game).toContain("const squeeze = 'scaleX(' + scale.toFixed(4) + ')'");
    ...
    expect(app).toContain('class="image-footer"');
    expect(app).toContain('Designed by EgawaHokori. Data from 水鱼查分器.');
    expect(app).toContain('<div>Generated by rRanker</div>');
  ...
  it('reduces the game trophy weight and squeezes its width when the name overflows the shougou plate', () => {
    const short = buildBestImageHtml({
      type: 'custom', width: 1080, rating: 0, scoreSections: [],
      fontUrl: 'data:font/ttf;base64,Zm9udA==', ratingFrameUrl: 'data:image/png;base64,aW1hZ2U=',
      player: { displayName: '玩家', presentation: { trophyName: '短称号' } },
    });
    expect(short).toContain('<div class="game-trophy-text" style="left:219px;top:133px;font-size:11px">短称号</div>');
    expect(short).not.toContain('game-trophy-text-compressed"');
    expect(short).toContain('.game-trophy-text-compressed{max-width:190px;overflow:hidden;text-overflow:ellipsis;font-weight:400}');

    const long = buildBestImageHtml({
      type: 'custom', width: 1080, rating: 0, scoreSections: [],
      fontUrl: 'data:font/ttf;base64,Zm9udA==', ratingFrameUrl: 'data:image/png;base64,aW1hZ2U=',
      player: { displayName: '玩家', presentation: { trophyName: '这是一个超级无敌长的称号文字用于验证压缩' } },
    });
    expect(long).toContain('<div class="game-trophy-text game-trophy-text-compressed" style="left:219px;top:133px;font-size:9px">这是一个超级无敌长的称号文字用于验证压缩</div>');
    expect(long).toContain('max-width:190px');

    const medium = buildBestImageHtml({
      type: 'custom', width: 1080, rating: 0, scoreSections: [],
      fontUrl: 'data:font/ttf;base64,Zm9udA==', ratingFrameUrl: 'data:image/png;base64,aW1hZ2U=',
      player: { displayName: '玩家', presentation: { trophyName: '测试称号文字太长需要缩小字号适应底板' } },
    });
    expect(medium).toContain('font-size:10px');

    const app = buildBestImageHtml({
      type: 'custom', width: 1080, rating: 0, ratingStyle: 'app', scoreSections: [],
      fontUrl: 'data:font/ttf;base64,Zm9udA==', ratingFrameUrl: 'data:image/png;base64,aW1hZ2U=',
      player: { displayName: '玩家', presentation: { trophyName: '这是一个超级无敌长的称号文字用于验证压缩' } },
    });
    expect(app).not.toContain('game-trophy-text-compressed');
    expect(app).toContain('>这是一个超级无敌长的称号文字用于验证压缩</div>');
  });
