...
- 说明文本保留 `~~内容~~` 的删除线语义，并在单标签通知和全部标签弹层中渲染；Markdown 链接降级为可读的链接文字。
...
- 当前谱面的完整 DXRating 标签名称同时作为该难度卡本地标签的固定预设；不写入或修改全局标签预设，历史本地标签仍可直接选择。
- 舞萌成绩页和曲库页可按全部三个分组的 DXRating 数字标签 ID 多选筛选；多选使用 AND 语义，谱面必须同时拥有全部已选标签。
- 曲库的类型、难度、定数、谱面版本与标签条件必须在同一张谱面上共同命中；成绩按规范化歌曲 ID、谱面类型和难度序号精确匹配，不按曲名或本地标签猜测。
