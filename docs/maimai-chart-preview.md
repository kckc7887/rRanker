...
同一脚本的 `audio-tail` 模式用 161 秒合成音频与音乐时间 156 秒处结束的谱面，检查
跨过谱尾继续播放、尾奏暂停恢复、最后 30 毫秒起播和自然结束；界面计入两秒引导拍，总长 2:43。
`chart-tail` 模式检查短音频自然结束后的连续计时与变速、长条尾段拖动和恢复，以及进度条终点。
两种模式默认随完整脚本运行，也可在 Playwright 模块参数后指定；它们不使用实际 raputa 资源。
