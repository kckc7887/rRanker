...
| 功能族 | 公共入口 | 游戏侧职责 | 主要验证 |
|---|---|---|---|
| 谱面确认 | `src/features/chart-preview-shared/`：`ChartPreviewScreenShell`、资源暂存、URI 解析、桥接、注入工厂、计划执行器、播放时钟与全屏锁 | 提供图表解析、资源清单、HTML/脚本配置和场景文案 | `chart-preview-screen-shell-contract.test.tsx` 及各游戏预览测试 |
